        ...
        <div class="card shadow">
            ...
            <div class="card-body">
                ...
                @endif
                    ...
                    @if($role == 'paciente')
                        <p>Se cancelará tú cita reservada con el personal trainer <b>{{ $appointment->doctor->name }}</b>
                            (especialidad <b>{{ $appointment->specialty->name }}</b> ) 
                            para el día <b>{{ $appointment->scheduled_date }}</b>. 
                        </p>
                    @elseif($role == 'doctor')
                    <p>Se cancelará la cita  del cliente <b>{{ $appointment->patient->name }}</b>
                        (especialidad <b>{{ $appointment->specialty->name }}</b> ) 
                        para el día <b>{{ $appointment->scheduled_date }}</b>
                    ...
                    @else
                    <p>Se cancelará la cita  del cliente <b>{{ $appointment->patient->name }}</b>
                        que será atendido por el personal trainer <b>{{ $appointment->doctor->name }}</b>
                        (especialidad <b>{{ $appointment->specialty->name }}</b> ) 
                        para el día <b>{{ $appointment->scheduled_date }}</b>
                        , la hora <b>{{ $appointment->scheduled_time_12 }}</b>. 
                    </p>
